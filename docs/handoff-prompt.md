...
- **Podium medals** — top-3 rank cells in `PoolStandingsTable.tsx` now render gold / silver / bronze medal badges (metallic gradients tuned for the dark theme, 1st has a soft glow) via a new `RankBadge` sub-component, replacing the old flat amber number. Pure standing/skill recognition — RG-safe (confirmed: leaderboard rank, not spend).
