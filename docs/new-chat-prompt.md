...
**Most recent state: step 3b.13 — outcome-recording integrity hardened (21 June 2026).** After a live incident (WC opener Spain 4-0 Saudi was recorded as 5-0 from a transient football-data value — a goal later disallowed by VAR — and stuck via first-write-wins), the results path was rebuilt for safety: **confirm-before-commit** (a finished score is buffered and only committed/scored once stable across sync passes), first-write-wins immutability kept, a **divergence alert** (Admin → Score alerts), and an audited manual **correction tool** (`server/scripts/correct-outcome.ts`). Full canon in **architecture §24**. Also this step: tsc baseline is now **0** (was 15); `.gitattributes` forces LF. New table `event_outcome_observations` (db:push already run).

**Before that, step 3b — Eliminator10, launched 20 June 2026.** A second game mode alongside the score-prediction pools: a last-survivor **elimination** game (pick one team to win each round; win in normal time survives; loss/draw/no-pick out; one team only once; last entrant wins). Full canon is **architecture §22**. Live for the WC as a **free** demo (`world-cup-2026-eliminator`, 24 rounds, Round 1 = the Spain matchday), built **PL-ready** (free now → real fee + 75/25 pot later). Trademark wording: an **"elimination game" / "outlast the field"**, never "last man standing".
...
- Don't relitigate locked decisions (see handoff "Decisions ... DO NOT relitigate"). Note: the P3 score-correction gap is **RESOLVED** as of step 3b.13 (confirm-before-commit + divergence alert + correction tool, arch §24) — don't describe it as open or deferred, and don't re-open the "never silent auto-overwrite" principle.
