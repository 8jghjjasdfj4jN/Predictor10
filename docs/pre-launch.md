...
- [x] **Governed player removal (built step 3b.14, arch §25).** Removing a player from a pool is an audited admin action (`POST /api/admin-portal/entries/:entryId/void`, reason required) that **voids** the entry rather than deleting it — the row, payment and audit trail are retained, satisfying the "records retained + removals ruled, not improvised" expectation. Good evidence for the application. **Still open (analogous):** the governed in-app **late-entry** action (replacing the raw-SQL shell habit) — same pattern, not yet built.
