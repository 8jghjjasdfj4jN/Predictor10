...
- **Steps 3a.12 – 3a.17** (WC price sync to £10, real-name + nickname split, KYC-name schema, WC-only marketing simplification with `.tsx.bak` preservation, in-app admin portal for the 3 founding admins, pre-WC P1/P2 production fixes — `pool_entries` unique index + `enterPool` 23505 catch, settlement-before-scoring gate — and the predict-screen lock-rejection display fix): ✅ — *full detail in `handoff-prompt.md` steps 3a.12–3a.17*
- **Step 3a.18** (Player-predictions view + live status + predict-feed reorder + honest footer — lock-gated "see another player's picks" via `GET /api/pools/:poolId/entries/:entryId/predictions` and `OpponentPredictionsPage` (tap a table row); pulsing red LIVE badge + "Locked · awaiting kick-off" status on both the player view and the Predict screen; Predict feed reordered to live → still-predictable → about-to-start → finished, day/stage headers replaced by per-row date + round; removed the "test mode / virtual credits / no real money" copy from `SiteFooter` / `AuthShell` / `RegisterPage`; P3 score-correction gap accepted/closed. All competition-agnostic — carries to PL/cups for free. See arch §18 + §15 notes. tsc baseline 18 → 15, zero new): ✅
- **Step 3a.19** (Engagement/"juice" session — (1) per-pool **table chat** as a TEMPORARY WC feature: `pool_messages` table, entrant-gated, plain text + emoji, admin-hide, rate-limit + self-exclusion gate, polling not websockets, sentinel-fenced for clean teardown (`wc-chat-teardown.md`); brings Online Safety Act / Ofcom scope — documented for licence. (2) **Pick distribution** "How the table called it" — locked-only, entrant-gated, inside each match card, "21/23 picks". (3) Predict-screen **live/timing treatments** — lock & kick-off countdowns (day-aware, amber pulse), dedicated live "Your pick" card, amber "STARTS SOON" treatment for imminent locked matches, and corrected feed order (live → about-to-start → recently-finished → still-predictable → played → awaiting). Live points-on-pace parked on cost (needs paid football-data livescores add-on ~€12/mo); reactions/head-to-head/music/cheer-audio considered and declined. See arch §19–§21. tsc baseline held at 15, zero new): ✅
- **Pre-Final task (not yet built)**: keep a settled pool visible on Home/Tables until manual retirement, so WC doesn't auto-vanish when the Final settles (operator-triggered removal). See arch §15 / handoff operational section. 🔜
- **WC chat retirement (step 3a.19)**: the per-pool chat is temporary — remove it at WC wind-down per `docs/wc-chat-teardown.md` (trigger: "Read the WC chat teardown doc and remove the chat"). 🔜
- **Live points-on-pace — parked on cost**: needs the paid football-data livescores add-on (~€12/mo); free tier scores are delayed. Revisit at scale/with revenue. Decision: arch §20 / pre-launch. 🔜
- **Chat → Online Safety Act compliance (if chat ships at licence)**: chat makes Predictor10 a "user-to-user service" (Ofcom) — risk assessment, reporting route, retention, CSAM duties. Licence-prep line item, not a code task. See pre-launch §3. 🔜
...
| 3a.16 | ✅ shipped | Pre-WC audit + two production fixes. **P1**: `pool_entries_pool_user_idx` unique index on `(pool_id, user_id)` + `enterPool` `23505` catch → "already entered" (closes the duplicate-entry race; was a pre-licence-grant blocker, now done). **P2**: settlement gate (`pool-settle.ts findReadyPoolIds`) gains a `NOT EXISTS` guard so a pool won't settle while any of its predictions on a FINISHED event is unscored (closes a sync/settle race that could score a correct Final prediction as 0). **P3** (FD score-correction) deliberately left as-is — no silent auto-overwrite; admin alert deferred. **P4** (sub-3-entrant prize display) dropped. Also: predict-screen lock note added to `PoolDetailPage.tsx`; two late entrants given opener predictions via shell (audit-tagged `admin-shell-late-entry`). `pnpm db:push` ran for the index; build green, zero new tsc errors |
| 3a.17 | ✅ shipped | Predict-screen lock-rejection display fix. A prediction edit attempted after the 1hr lock was correctly rejected server-side (403, DB unchanged) but the client left the typed value on screen as a phantom. `PredictMatchRow.tsx` now reverts inputs to the saved pick on a 403; the row then refetches to its locked read-only state. Client-only, no schema. Confirmed cosmetic — no scoring or money impact; DB always held the correct value |
...
- **Sync/settle are independent crons that can overlap (3a.16)**: outcome-write and prediction-scoring in `outcome-sync.ts` aren't transactional, so a settle pass could observe a finished event with unscored predictions and count them 0. Fixed with the gate guard. Lesson: any future "X is ready when all of Y is done" gate should also assert that the *derived* values of Y (here, `points_awarded`) are populated, not just that the raw rows (outcomes) exist.
- **Lock exceptions are currently ad-hoc (3a.16)**: late entrants were let past the 1-hour lock by raw shell SQL. Acceptable for the informal run, but pre-licence this needs a governed in-app admin action with a reason field and audit row — a regulator expects lock exceptions to be ruled, not improvised at the DB.
...
## Step 3b.15 — Score-alert scoping + after-settlement correction gap named (June 2026) — BUILT

The divergence detector + Admin → Score alerts panel are now **pool-scoped** (arch §24.1). The detector only raises for a match in a pool (open/locked/settled); the panel shows **only divergences on a live pool (open/locked)**, tagged with competition + Round, with a plain "affects a live pool — correct before it settles" line. No-pool and settled-pool divergences are hidden (settled still logged for the audit trail). This cleared a standing false-positive — a season-over PL fixture (Man City v Aston Villa, stored 2-2 vs football-data 1-2) on no live pool — that was cluttering the panel. Code-only (no schema/seed). tsc 0, build green, crossorigin intact.

Also **named** (not built): correcting a result re-scores every player up and down and the table self-heals **before settlement** (arch §24.2). There is **no after-settlement reversal** (recompute ranks + reverse payouts); the tool refuses a settled pool unless forced. Fine for free play; a **named pre-licence-grant task** for real money — see pre-launch §3.

## Step 3b.16 — In-panel score correction (June 2026) — BUILT

Corrections can now be applied **from the admin panel**, not just the CLI (arch §24.2). Each live-pool score alert gets a **"Correct score"** button → a two-step modal: enter the real score → **Preview** (shows exactly which players gain/lose points, nothing written) → required reason → **Apply**. Deliberate, human-initiated, audited — **not** a silent auto-overwrite (the locked §24 rule stands: the feed still can't change a result, only a person can, on the record). Panel refuses a settled pool (409); never forces. The CLI (`correct-outcome.ts`) and the panel now share one implementation — `server/lib/outcome-correction.ts` (`previewCorrection` + `applyCorrection`) — so the re-score can't drift; the CLI was refactored onto it. Endpoints: `POST /score-alerts/preview` + `POST /score-alerts/correct`. Correction audit rows carry `metadata.kind: "outcome_correction"` (also how "resolved" is detected). Solved the real need: fixing WC results (e.g. Portugal, Belgium) from the phone instead of Render Shell. Code-only (no schema/seed). tsc 0, build green, crossorigin intact.
